...
import { fetchHelpdeskThread } from "./helpdesk";
import { logger } from "./logger";
import { prisma } from "./prisma";
import type { TicketProcessingJob } from "./queue";
import {
...
export const processTicketProcessingJob = async (
  data: TicketProcessingJob,
): Promise<void> => {
  const startedAt = Date.now();
  const { merchantId, ticketId, helpdeskTicketId } = data;
...
export const processTicketJob = async (
  job: Job<TicketProcessingJob>,
): Promise<void> => {
  await processTicketProcessingJob(job.data);
};
